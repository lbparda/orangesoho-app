...
export function useOfferCalculations(
    ...
    const operadoraAutomaticaInfo = computed(() => { // Necesario para cálculo Operadora
     ...
     const appliedDiscount = computed(() => { // Copiado literal de tu original
    ...
    // --- CALCULATION SUMMARY (Copiado literal de tu original) ---
    const calculationSummary = computed(() => {
        ...
        // Usar los nombres de categoría EXACTOS de tu original
        ...
        // Lógica Operadora Automática original
        ...
        if (isCentralitaActive.value) {
            ...
            if (autoIncludedExtension.value) {
                commissionDetails.Centralita.push({ description: `1x ${autoIncludedExtension.value.name} (Por Centralita)`, amount: 0 }); // Original tenía 0 comisión aquí
            }
            ...
            for (const addonId in centralitaExtensionQuantities.value) {
                ...
                if (quantity > 0) {
                    ...
                    if (addonInfo) {
                        ...
                        price += itemPrice;
                        summaryBreakdown.push({ description: `${quantity}x ${addonInfo.name} (Adicional)`, price: itemPrice }); // Descripción original
        ...
        // Lógica Líneas Móviles y Terminales original
        ...
        if (currentUser.role === 'admin') {
            ...
            if (currentUser.role === 'user') {
                const userPercentage = currentUser.commission_percentage || 0; // Usar comisión individual del usuario
                userCommission = teamCommission * (parseFloat(userPercentage) / 100);
            } else { // team_lead
                userCommission = teamCommission; // El jefe de equipo se lleva la del equipo
            }
        }
        else { // user sin equipo
            const userPercentage = currentUser.commission_percentage || 0; // Usar comisión individual del usuario
            userCommission = totalCommission * (parseFloat(userPercentage) / 100);
            teamCommission = 0; // No hay equipo
        }

        // Devolver exactamente la misma estructura que tu original
        return {
